...
#include <string.h>
#include <unistd.h>
#define READ_THHREAD_NUM 10
#define WRITE_THHREAD_NUM 5
/**
 * gdb 调试多线程程序
 * info threads ：展示所有线程
 * thread n ：切换到第n个线程
...
void *read_function(void *argv){
    ...
    sleep(1);
    printf("thread %ld read_data: %d read_str: %s\n",pthread_self(), data.num,data.str);
...
int main(){
    ...
    pthread_t read_thread[READ_THHREAD_NUM];
    pthread_t write_thread[WRITE_THHREAD_NUM];
    int i;
    for(i = 0; i < READ_THHREAD_NUM; i++){
        pthread_create(&read_thread[i], NULL, read_function,NULL);
    }
    for(i = 0; i < WRITE_THHREAD_NUM; i++){
